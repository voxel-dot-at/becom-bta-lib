...
// param len :number of elements á 16 bit
unsigned short crc16_ccitt_in_16bit(const unsigned short *buf, int len)
{
    register int counter;
    register unsigned short crc = 0;
    register unsigned short usTemp = 0;
    for( counter = 0; counter < len; counter++) {
        usTemp = *buf++;
        crc = (crc<<8) ^ crc16tab[((crc>>8) ^ (usTemp >> 8))&0x00FF];
        crc = (crc<<8) ^ crc16tab[((crc>>8) ^ (usTemp))&0x00FF];
    }
    return crc;
}

unsigned short CRCCCITT(unsigned char *data, int length, unsigned short seed, unsigned short final)
{
  register int count;
  register unsigned int crc = seed;
