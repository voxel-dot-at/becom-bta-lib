...
static void MtSync_StopWriting(CMtSync *p)
{
  ...
  */
  ...
  Semaphore_Release1(&p->freeSemaphore); // check semaphore count !!!
    ...
    PRF(println("\nMtSync_StopWriting %p : Event_Wait(&p->wasStopped)", p));
  Event_Wait(&p->wasStopped);
    PRF(println("\nMtSync_StopWriting %p : Event_Wait() finsihed", p));
...
static void HashThreadFunc(CMatchFinderMt *mt)
{
  ...
  for (;;)
  {
    ...
    for (;;)
    {
      PRF(println("Hash thread block = %d pos = %d", (unsigned)blockIndex, mt->MatchFinder->pos));
...
static void BtThreadFunc(CMatchFinderMt *mt)
{
  ...
  for (;;)
  {
    ...
    for (;;)
    {
        PRF(println("  BT thread block = %d  pos = %d", (unsigned)blockIndex, mt->pos));
